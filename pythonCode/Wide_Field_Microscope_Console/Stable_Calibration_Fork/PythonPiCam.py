...
- Limits on the allowed postion of the stepper motors to avoid damaging the 
    microscope
...
- Show a preview of the current view in the image plane
...
directory_self = str(os.getcwd())
directory_save = str(os.getcwd())
os.chdir(directory_save)

# Define a function to print the current directories.

def PrintCurrentDirectories():
    print ('\nThe current self directory is: ' + directory_self)
    print ('\nThe current save directory is: ' + directory_save + '\n')
...
# Define a function that prints the current camera settings.

def PrintCurrentCameraSettings():
    print ('\nResolution: ' + str(camera_resolution))
    print ('Preview Length: ' + str(preview_length))
    print ('')
...
# out because it is not being called anywhere else. Was used for debugging puprposes.
...
# Define a function to print the current motor settings.

def PrintCurrentMotorSettings():
    print ('\nThe small step size is ' + str(small_step_size) + ' steps.')
    print ('The large step size is ' + str(large_step_size) + ' steps.')
    print ('The step pause is ' + str(step_pause*1000) + ' milliseconds.\n')
...
# This function moves the motors in order of x then y then z to get to the destination,
# and is called with an array of three values representing the (x, y, z) coordinate of
# the destination.
...
def MoveToPosition(new_position):
    ...
    if move_to_position_valid == True:
        ...
        print ('Select a new position within the allowable boundaries and try again.')

# Define a function that allows the user to move to any (x, y, z) coordinate within the
# allowable range of motion. Relies on the MoveToPosition(new_position) function to 
# perform the test on whether or not the new position is allowable.
       
def UserDefinedPosition():

    user_new_position = [0,0,0]
    for i in range(0,3):
        user_new_position[i] = int(input('New ' + motorMap[i] + ' position: '))
...
The motorMap dictionary can be used to simplify the exchange between (0,1,2) <--> (x,y,z). 
...
def MotorMove(axis, steps, time_between_steps):
    ...
    # The time_between_steps parameter can be used to control
    # effective motor rotation speed. 
    ...
    # prevent the motor from destroying the microscope by simply rotating until 
...
# Function for performing a one dimensional scan of the sample.

#def OneDimensionalScan(direction,min_scan_value,max_scan_value,scan_method):
#    
#    print ('You\'ve selected a scan in the ' + str(direction + ' direction.')
#    print ('Showing a preview of your image region...')
#    sleep (1)
#    Preview(3)
#    sleep (1)
#    
...
#        
#        z_max = int(input('Please enter the desired start position of the scan: '))
#        z_min = int(input('Please enter the desired end position of the scan: '))
#            
#        if z_max <= z_min:
...
# Function for the user to perform a one dimensional scan.

#def UserDefinedOneDimensionalScan():
#        desired_direction = str(input('Enter the desired scan direction (x, y, or z): '))
...
#        desired_scan_method = str(input('Do you want to define the number of images in the z scan (N), or the spacing between each image (S)?'))
#        OneDimensionalScan(desired_direction,desired_min_scan,desired_max_scan,desired_scan_method)
...
while True:
    ...
    # If the help menu is requested, display the help menu using DisplayMenu(menu).
    ...
    # Move at a speed of rotation according to the value of step_pause.
        ...
        MotorMove(motorMap['x'],-small_step_size,step_pause)         
        PrintPosition()
        UpdatePositionFile()
        ...
        MotorMove(motorMap['y'],small_step_size,step_pause)
        PrintPosition()
        UpdatePositionFile()
        ...
        MotorMove(motorMap['z'],-small_step_size,step_pause)
        PrintPosition()
        UpdatePositionFile()
    ...
    # Move at a speed of rotation according to the value of step_pause.
        ...
        MotorMove(motorMap['x'],-large_step_size,step_pause)
        PrintPosition()
        UpdatePositionFile()
        ...
        MotorMove(motorMap['y'],large_step_size,step_pause)
        PrintPosition()
        UpdatePositionFile()
        ...
        MotorMove(motorMap['z'],-large_step_size,step_pause)
        PrintPosition()
        UpdatePositionFile()
    ...
    # later time. Provide a function for returning to the origin at any
    ...
    # Provide a function for returning to the origin at any time.
    
    elif user_command == 'B':
        ReturnToOrigin()
        PrintPosition()

    # Provide the user a way to see what the currently saved position is.    
    
    elif user_command == 'K':
        PrintSavedPosition()
    
    # Provide the user a way to move to an arbitrary allowable position within the     
    # microscope stage range
    
    elif user_command == 'R':
        UserDefinedPosition()
        
    # Provide the user direct access to the OneDimensionalScan function.
    
#    elif user_command == 'O':
#        
#        UserDefinedOneDimensionalScan()
            
    # Provide the user a way to change the self and save directories. 
    # The self directory keeps the all of the relevant text
    ...
    # Also allow the user to see the currently defined directories.
    ...
    # is given in milliseconds.
        ...
        if motor_menu_choice == 'S':
            small_step_size = int(input("Enter the new desired small step size: "))
        elif motor_menu_choice == 'L':
            large_step_size = int(input("Enter the new desired large step size: "))
        elif motor_menu_choice == 'T':
            desired_step_pause = int(input("Enter the new pause between steps (must be between 1 & 100): "))
            if desired_step_pause >= 1 and desired_step_pause <= 100:
                step_pause = desired_step_pause/1000
            else:
                print ("Invalid pause between steps.")
        elif motor_menu_choice == 'D':
            PrintCurrentMotorSettings()
        else:
    ...
    # camera_menu.txt that outlines what the options are.
        ...
        if camera_menu_choice == 'D':
            PrintCurrentCameraSettings()